...
xmlrpc_client_call(xmlrpc_env * const envP,
    ...
    xmlrpc_value ** resultP = malloc(number_of_servers * sizeof(xmlrpc_value *));
	...
	for(i=0; i<number_of_servers; i++)
        	xmlrpc_client_call2f_va(envP, globalClientP, serverUrl[i],
                 	               methodName, format, &resultP[i], args);
...
void 
xmlrpc_client_call_asynch(xmlrpc_env * const envP,
	...
	{
        	xmlrpc_client_start_rpcf_va(envP, clientP,
                  	        	    serverUrl[i], methodName,
                        	            responseHandler, userData,
                                	    format, args);
	}
